...
public class RareEpicItems {
    ...
    public static void registerItems(CustomWeaponEngine plugin) {
        ...
        // --- WEAPONS ---
        // Golem Sword (Rare)
        createWeapon(plugin, lib, "cwe_golem_sword", "Golem Sword", "RARE", Material.IRON_SWORD, 80, 125, 0, 0, 0, 0, 0,
                "Iron Punch", "Nện kiếm giật khối sắt xung quanh.", "", "RIGHT CLICK");

        // Raider Axe (Rare)
        createWeapon(plugin, lib, "cwe_raider_axe", "Raider Axe", "RARE", Material.IRON_AXE, 80, 50, 0, 0, 0, 0, 0,
                "Thợ Săn Tiền Thưởng", "Tăng lượng tiền rớt ra khi giết quái.", "", "PASSIVE");

        // Zombie Sword (Epic)
        createWeapon(plugin, lib, "cwe_zombie_sword", "Zombie Sword", "EPIC", Material.IRON_SWORD, 100, 50, 0, 0, 0, 0, 100,
                "Instant Heal", "Hồi 10% máu cho bản thân và 5% cho", "đồng minh xung quanh (Tốn 50 Mana).", "RIGHT CLICK");

        // Pigman Sword (Legendary)
        createWeapon(plugin, lib, "cwe_pigman_sword", "Pigman Sword", "LEGENDARY", Material.GOLDEN_SWORD, 200, 100, 0, 0, 20, 50, 300,
                "Burning Souls", "Bắn ra ngọn lửa linh hồn lợn gây nổ,", "tăng 50 Defense trong 5 giây.", "RIGHT CLICK");

        // Fel Sword (Epic)
        createWeapon(plugin, lib, "cwe_fel_sword", "Fel Sword", "EPIC", Material.IRON_SWORD, 130, 0, 0, 0, 0, 0, 0,
