...
public class ItemStatsListener implements Listener {
    ...
    private double[] collectTotals(Player player) {
        ...
        if (mainHand != null && !mainHand.getType().isAir()) {
            ...
            if (!isArmorItem) {
                ...
                // WITHERED: cộng thêm levelMultiplier * AuraSkills Fighting Level vào strength
                    ...
                    if (pdc.has(new NamespacedKey(plugin, "cwe_reforge"), PersistentDataType.STRING)) {
                        ...
                        if (bonus != null && bonus.levelMultiplier > 0) {
                            ...
                            if (org.bukkit.Bukkit.getPluginManager().isPluginEnabled("AuraSkills")) {
                                ...
                                if (u != null) {
                                    lvl = u.getSkillLevel(dev.aurelium.auraskills.api.skill.Skills.FIGHTING);
                                }
                            }
                            mainHandTotals[0] += bonus.levelMultiplier * lvl;
                        }
                    }
                }
                ...
                if (offHand.hasItemMeta()) {
                    ...
                    if (pdc.has(new NamespacedKey(plugin, "cwe_reforge"), PersistentDataType.STRING)) {
                        ...
                        if (bonus != null && bonus.levelMultiplier > 0) {
                            ...
                            offHandTotals[0] += bonus.levelMultiplier * lvl;
                        }
                    }
                }
    ...
    public void onPlayerTakeDamage(org.bukkit.event.entity.EntityDamageEvent event) {
        if (event.getEntity() instanceof Player) {
            ...
            double defense = 0;
            if (cached != null) {
                defense = cached[4]; // 4 is defense
            }
            if (p.hasMetadata("cwe_temp_defense")) {
                for (org.bukkit.metadata.MetadataValue meta : p.getMetadata("cwe_temp_defense")) {
                    if (meta.getOwningPlugin().getName().equalsIgnoreCase(plugin.getName())) {
                        defense += meta.asDouble();
                    }
                }
            }
            if (defense > 0) {
